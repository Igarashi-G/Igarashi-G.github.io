...
<span class="line"><span style="--shiki-light:#383A42;--shiki-dark:#ABB2BF;">RELEASE SAVEPOINT 名称;   </span><span style="--shiki-light:#A0A1A7;--shiki-light-font-style:italic;--shiki-dark:#7F848E;--shiki-dark-font-style:italic;">-- 释放保存点</span></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_3-隔离级别对比" tabindex="-1"><a class="header-anchor" href="#_3-隔离级别对比"><span>3. 隔离级别对比</span></a></h3><table><thead><tr><th style="text-align:center;">隔离级别</th><th style="text-align:center;">脏读</th><th style="text-align:center;">不可重复读</th><th style="text-align:center;">幻读</th><th style="text-align:center;">实现机制</th></tr></thead><tbody><tr><td style="text-align:center;">READ UNCOMMITTED</td><td style="text-align:center;">可能</td><td style="text-align:center;">可能</td><td style="text-align:center;">可能</td><td style="text-align:center;">无防护</td></tr><tr><td style="text-align:center;">READ COMMITTED</td><td style="text-align:center;">不可能</td><td style="text-align:center;">可能</td><td style="text-align:center;">可能</td><td style="text-align:center;">MVCC快照读</td></tr><tr><td style="text-align:center;">REPEATABLE READ</td><td style="text-align:center;">不可能</td><td style="text-align:center;">不可能</td><td style="text-align:center;">可能*</td><td style="text-align:center;">MVCC+间隙锁</td></tr><tr><td style="text-align:center;">SERIALIZABLE</td><td style="text-align:center;">不可能</td><td style="text-align:center;">不可能</td><td style="text-align:center;">不可能</td><td style="text-align:center;">完全串行化</td></tr></tbody></table><div class="hint-container tip"><p class="hint-container-title">关于 <strong>REPEATABLE READ</strong> 和幻读</p><p><strong>MySQL</strong> 的 <strong>InnoDB</strong> 在 <strong>REPEATABLE READ</strong> 隔离级别下，通过 <strong>间隙锁</strong> 解决了部分幻读，但没有完全解决快照读下的幻读问题</p><p>可以做这个实验：</p><ol><li>当前DB已有id 5, 10, 15三条数据。</li><li>事务A查询id &lt; 10的数据，可以查出一行记录id = 5</li><li>事务B插入id = 6的数据</li><li>事务A再查询id &lt; 10的数据，可以查出一行记录id = 5，查不出id = 6的数据（读场景，解决了幻读）</li><li>事务A可以更新/删除id = 6的数据，不能插入id = 6的数据（写场景，幻读不彻底）</li></ol><h4 id="实验场景复现" tabindex="-1"><a class="header-anchor" href="#实验场景复现"><span>实验场景复现</span></a></h4><div class="language-sql line-numbers-mode" data-highlighter="shiki" data-ext="sql" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34;"><pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span style="--shiki-light:#A0A1A7;--shiki-light-font-style:italic;--shiki-dark:#7F848E;--shiki-dark-font-style:italic;">-- 初始数据</span></span>
...
七、锁
    ...
    9.优化SQL的方法：
        ...
        6.适用外键，优化锁定表
        7.建立索引
</code></pre><h2 id="快照读没有完全解决幻读" tabindex="-1"><a class="header-anchor" href="#快照读没有完全解决幻读"><span>快照读没有完全解决幻读？</span></a></h2><p>是的，MySQL没有完全解决快照读下的幻读问题。</p><p>可以做这个实验：</p><ol><li>当前DB已有id 5, 10, 15三条数据。</li><li>事务A查询id &lt; 10的数据，可以查出一行记录id = 5</li><li>事务B插入id = 6的数据</li><li>事务A再查询id &lt; 10的数据，可以查出一行记录id = 5，查不出id = 6的数据（读场景，解决了幻读）</li><li>事务A可以更新/删除id = 6的数据，不能插入id = 6的数据（写场景，幻读不彻底）</li></ol><p>这个很好理解，MySQL虽然通过MVCC的版本号来解决了读场景下的幻读，但对于上面第5步那种写场景的情况，其实是无能为力的，因为MVCC毕竟是无锁实现。</p><p>所以如果后续要对数据进行写操作，还是<strong>通过for update语句上锁</strong>比较稳妥，不然就可能会出现上面第5步那样的问题。</p></div>`,72)]))}const p=s(l,[["render",h]]),c=JSON.parse('{"path":"/database/MySQL/Mysql%E8%B0%83%E4%BC%98.html","title":"Mysql调优","lang":"zh-CN","frontmatter":{"title":"Mysql调优","order":4,"description":"MYSQL 机制详解 MySQL 执行查询的过程 客户端通过 TCP 连接发送连接请求到 MySQL 连接器，连接器会对该请求进行权限验证及连接资源分配 查缓存。（当判断缓存是否命中时，MySQL 不会进行解析查询语句，而是直接使用 SQL 语句和客户端发送过来的其他原始信息。所以，任何字符上的不同，例如空格、注解等都会导致缓存的不命中。） 语法分析（...","head":[["script",{"type":"application/ld+json"},"{\\"@context\\":\\"https://schema.org\\",\\"@type\\":\\"Article\\",\\"headline\\":\\"Mysql调优\\",\\"image\\":[\\"\\"],\\"dateModified\\":\\"2025-06-13T06:08:46.000Z\\",\\"author\\":[{\\"@type\\":\\"Person\\",\\"name\\":\\"悦·宝宝\\",\\"url\\":\\"https://github.com/Igarashi-G\\"}]}"],["meta",{"property":"og:url","content":"https://igarashi-g.github.io/database/MySQL/Mysql%E8%B0%83%E4%BC%98.html"}],["meta",{"property":"og:site_name","content":"悦 ▪ 宝宝"}],["meta",{"property":"og:title","content":"Mysql调优"}],["meta",{"property":"og:description","content":"MYSQL 机制详解 MySQL 执行查询的过程 客户端通过 TCP 连接发送连接请求到 MySQL 连接器，连接器会对该请求进行权限验证及连接资源分配 查缓存。（当判断缓存是否命中时，MySQL 不会进行解析查询语句，而是直接使用 SQL 语句和客户端发送过来的其他原始信息。所以，任何字符上的不同，例如空格、注解等都会导致缓存的不命中。） 语法分析（..."}],["meta",{"property":"og:type","content":"article"}],["meta",{"property":"og:locale","content":"zh-CN"}],["meta",{"property":"og:updated_time","content":"2025-06-13T06:08:46.000Z"}],["meta",{"property":"article:modified_time","content":"2025-06-13T06:08:46.000Z"}]]},"git":{"createdTime":1665586687000,"updatedTime":1749794926000,"contributors":[{"name":"Igarashi","username":"Igarashi","email":"2545369032@qq.com","commits":1,"url":"https://github.com/Igarashi"},{"name":"Igarashi-G","username":"Igarashi-G","email":"2545369032@qq.com","commits":1,"url":"https://github.com/Igarashi-G"}]},"readingTime":{"minutes":35.78,"words":10735},"filePathRelative":"database/MySQL/Mysql调优.md","excerpt":"\\n<h4>MySQL 执行查询的过程</h4>\\n<ol>\\n<li>客户端通过 TCP 连接发送连接请求到 MySQL 连接器，连接器会对该请求进行权限验证及连接资源分配</li>\\n<li>查缓存。（当判断缓存是否命中时，MySQL 不会进行解析查询语句，而是直接使用 SQL 语句和客户端发送过来的其他原始信息。所以，任何字符上的不同，例如空格、注解等都会导致缓存的不命中。）</li>\\n<li>语法分析（SQL 语法是否写错了）。 如何把语句给到预处理器，检查数据表和数据列是否存在，解析别名看是否存在歧义。</li>\\n<li>优化。是否使用索引，生成执行计划。</li>\\n<li>交给执行器，将数据保存到结果集中，同时会逐步将数据缓存到查询缓存中，最终将结果集返回给客户端。</li>\\n</ol>","autoDesc":true}');export{p as comp,c as data};
