<template><div><p>接上文，查找重点：</p>
<pre><code>&lt;4&gt; 查（filter，value等)：
     查询相关API：
...
&lt;2&gt; 可切片： 由于 QuerySet 继承列表因此可进行常规的列表操作</p>
<pre><code>惰性机制：
...
<p>四、对象查询，单表条件查询，多表条件关联查询： 1.对象形式的查找：！！！！！！！！！！！！！！！！！！(通过.的方式)
正向查找:（注意，这种方式只适用于一对一，一旦出现多！无法获取，见下）
ret1=models.Book.objects.first()
print(<a href="http://ret1.publisher.name" target="_blank" rel="noopener noreferrer">ret1.publisher.name<ExternalLinkIcon/></a>) #多对一绑定的外键，publisher 是单一对象，一本书对应一个出版商，<a href="http://xn--gzus34b.name" target="_blank" rel="noopener noreferrer">直接.name<ExternalLinkIcon/></a> 即可取值 #但要小心一个出版商可对应着多本书,因此 Pobj.book.title 是无法获取书名的,一对多了！-error
#Pobj.book_set.all().values(&quot;title&quot;)只能用.all().values()获取多个出版书籍</p>
<pre><code>    反向查找:（_set）
...
<p>五、聚合查询和分组查询（了解）
聚合查询和分组查询都是聚合函数，区别在于 aggregate 是在结果集上做操作处理一个对象。而分组则是处理多个对象</p>
<pre><code>&lt;1&gt; aggregate(*args,**kwargs):
...
<p>六、F 查询和 Q 查询：（重点！灵活掌握）
仅仅靠单一的关键字参数查询已经很难满足查询要求。此时 Django 为我们提供了 F 和 Q 查询：
（若想要某字段的所有值全部+20）常规的.all().update(price=&quot;price&quot;+20???)这样无法满足，因此利用 F</p>
<pre><code>F 使用查询条件的值,专门取对象中某列值的操作：
...
<p>七、raw sql：
django 中 models 的操作,也是调用了 ORM 框架来实现的,pymysql 或者 mysqldb,所以我们也可以使用原生的 SQL 语句来操作数据库!</p>
...
<p>八、ORM 无法解决的问题：
场景：找出每个作者出版的所以书的最高价格，以及最高价格那本书的名称：</p>
<pre><code>通常我们会想到分组，然后利用max进行查找。
...
<p>九 admin 的配置：（定制性太强，玩转了 admin 才能真正帮助到你，此时不必深究，所有的东西都能通过 admin 实现）
admin 是 django 强大功能之一，它能共从数据库中读取数据，呈现在页面中，进行管理。默认情况下，它的功能已经非常强大，如果你不需要复杂的功能，
它已经够用，但是有时候，一些特殊的功能还需要定制，比如搜索功能，下面这一系列文章就逐步深入介绍如何定制适合自己的 admin 应用。</p>
<pre><code>如果你觉得英文界面不好用，可以在setting.py 文件中修改以下选项
...
<p>十、ORM 的锁和事务：
<a href="https://www.cnblogs.com/clschao/articles/10463267.html" target="_blank" rel="noopener noreferrer">https://www.cnblogs.com/clschao/articles/10463267.html<ExternalLinkIcon/></a></p>
<pre><code>一、锁：
...
<p>1 全局开启
　　　　在 Web 应用中，常用的事务处理方式是将每个请求都包裹在一个事务中。这个功能使用起来非常简单，你只需要将它的配置项 ATOMIC_REQUESTS 设置为 True。</p>
<p>它是这样工作的：当有请求过来时，Django 会在调用视图方法前开启一个事务。如果请求却正确处理并正确返回了结果，Django 就会提交该事务。否则，Django 会回滚该事务。
DATABASES = {
'default': {
...
　　　　　　　#'init_command': &quot;SET sql_mode='STRICT_TRANS_TABLES'&quot;, #配置开启严格 sql 模式</p>
<pre><code>            }
...
<p>被 atomic 管理起来的代码块还可以内嵌到方法中。这样的话，即便内部代码块正常运行，如果外部代码块抛出异常的话，它也没有办法把
它的修改提交到数据库中。</p>
<p>用法 1：给函数做装饰器来使用</p>
