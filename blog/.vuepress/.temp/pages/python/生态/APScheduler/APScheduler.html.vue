...
<p>[方式三]：</p>
<pre><code>init_scheduler_options = {
    &quot;jobstores&quot;: {
...
<p>如果有多个任务序列的话可以给每个任务设置 ID 号，可以根据 ID 号选择清除对象，且 remove 放到 start 前才有效</p>
...
<p>或者使用 print_jobs()来输出所有格式化的作业列表。如：</p>
...
<p>加入到某个任务触发后可能会触发异常：RuntimeError: cannot join current thread</p>
...
<p>导入 from apscheduler.events import EVENT_JOB_EXECUTED, EVENT_JOB_ERROR</p>
<pre><code>def my_listener(event)  # 设置一个监听任务，用于监听是否有任务异常
    if event.exception:
        print(&quot;任务出错了！！！&quot;)
    else:
        print(&quot;任务照常运行...&quot;)
