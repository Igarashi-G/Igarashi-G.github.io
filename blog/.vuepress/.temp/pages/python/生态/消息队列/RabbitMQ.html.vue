...
<p>二、实现队列消息的安全分发</p>
<pre><code>问题：当队列中的消息发送给消费者，消费者死了，怎么办？  -- 即消费者到包子店吃包子，吃一半噎死了，怎么办？
...
<p>三、实现队列&amp;队列消息的持久化</p>
<pre><code>问题：生产者发布了一个消息，此时消息队列如果挂了，怎么办？ -- 即包子铺着火了，rabbitmq宕机了，怎么办。（包子铺不在了，如何保住包子）
...
<p>七、Remote procedure call (RPC)：</p>
<pre><code>场景：模拟远程SSH，SSHClient（生产者p） 向 SSHServer（消费者c）发送消息 通过ssh_cmd队列。执行完之后结果返回。
