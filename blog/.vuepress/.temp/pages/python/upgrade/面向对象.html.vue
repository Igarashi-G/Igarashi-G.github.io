...
<p>python 会根据模板对象生成一个对象</p>
<pre><code>当创建一个类，之后进行实例化时 在内存中开辟了一个新的空间 并用“类对象指针”指向创建的类 因此可以访问类内方法
...
<ul>
<li>
<p>Beyond，例如技术词汇 metadata，意思是描述数据的超越数据。</p>
</li>
<li>
<p>Change，例如技术词汇 metamorphosis，意思是改变的形态。</p>
</li>
</ul>
...
<ul>
<li>
<p>调用顺序 父类 <strong>new</strong> -&gt; 父类 <strong>init</strong> -&gt; 父类 <strong>call</strong> -&gt; 子类 <strong>new</strong> -&gt; 子类 <strong>init</strong></p>
</li>
<li>
<p>父类 <strong>init</strong> 中的 cls, name, bases, dic 分别对应子类 &lt;class '<strong>main</strong>.Foo'&gt;, Foo, (), Foo 内部字典（不是<strong>dict</strong>）</p>
</li>
<li>
<p>mcs：这个才相当与父类的 self，它指向了当前父类本身</p>
</li>
</ul>
...
<ul>
<li>此时称 obj 为 Foo 类的对象 那么 Foo 类又是谁的对象。Foo 是 type 的对象，因此 Foo 不过是 class 开头的对象</li>
</ul>
<p>c.</p>
<ul>
<li>
<p>类其实都是 type 类的对象，实际类的对象: type()</p>
<pre><code>因此以上等同于 Foo = type()
</code></pre>
<p>#######################</p>
...
<p>得到结果：</p>
<pre><code>&lt;class '__main__.MyClass'&gt;
&lt;class 'type'&gt;
&lt;class 'type'&gt;
</code></pre>
...
<ul>
<li>
<p>classname：第一个参数，类名</p>
</li>
<li>
<p>superclasses：第二个参数，当前类的基类</p>
</li>
<li>
<p>attributedict：第三个参数，类的成员 (此处用 func 代替)</p>
<p>Foo().func() # 类 是由 type 类实例化产生 上文通过 type 自定义创建一个类 才是类实际的创建方式</p>
</li>
</ul>
<p>因此 metaclass ———— 类的祖宗 (普通类都是 type 创建的, 那么普通类都是 type 类的实例化对象 )</p>
<pre><code>即：  Foo = type()
</code></pre>
...
<p>其实每定义一个类，python 解释器实质上执行了如下代码：</p>
<pre><code>class MyClass:
    data = 1

class = type(classname, superclasses, attributedict)      # 见上文 type 类
</code></pre>
<p>它相当于是实例化了一个 type 类（上文 Baz 同理）</p>
<p>这里等号右边的 type(classname, superclasses, attributedict)，就是 type 的 <strong>call</strong> 运算符重载，它会进一步调用：</p>
...
<p>这相当于拿到了 type 创建类的权限，可以最大限度的改写，好用，也十分危险</p>
<p>【注意】：</p>
<ul>
<li>metaclass 仅仅是给小部分 Python 开发者，在开发框架层面的 Python 库时使用的。而在应用层，metaclass 往往不是很好的选择。</li>
</ul>
<p>-metaclass 是黑魔法，使用得当就是天堂，反之就是地狱。</p>
</div></template>
