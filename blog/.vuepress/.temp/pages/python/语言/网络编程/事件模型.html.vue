...
<p>—————————————————————————————————############################################———————————————————————————————————————————————</p>
<p>IO 多路复用:<br>
前面是用协程实现的 IO 阻塞自动切换，那么协程又是怎么实现的，其原理是怎么实现的。如何去实现事件驱动的情况下 IO 的自动阻塞的切换，这个学名<br>
叫什么呢？ =&gt; IO 多路复用<br>
比如 socketserver，多个客户端连接，单线程下实现并发效果，就叫多路复用。</p>
<pre><code>一、IO模型前戏准备：
    在进行解释之前，首先要说明几个概念：
...
<p>—————————————————————————————————############################################———————————————————————————————————————————————<br>
select 是怎么玩的？到底 select 为什么要叫 event driven IO（事件驱动 IO）？</p>
<p>select poll epoll IO 多路复用介绍：（监听多个文件描述符并实现并发）</p>
<pre><code>1、select，poll，epoll都是IO多路复用的机制。
...
<p>IO 多路复用进一步封装 selector 库的应用： 1.让其自动根据操作系统判断最优选择：用 select、poll 还是 epoll -- win 上只有 select<br>
sel = selectors.DefaultSelector()：</p>
<pre><code>2.绑定、监听、设置非阻塞、注册！！，重点是注册
