...
<li>
<p>loop.call_later(time, callback, *args): 指定时间之后再运行，执行的顺序和指定的时间有关（非线程安全的异步 API）</p>
</li>
<li>
<p>loop.call_at(loop.time() + x, callback, *args): 也是指定时间后执行，但这里的时间为 loop 里面的时间（非线程安全的异步 API）</p>
</li>
<li>
<p>loop.call_soon_threadsafe(callback, *args)： 用于调度不同 OS 线程的回调函数</p>
</li>
</ul>
<p>###3.新线程协程：run_coroutine_threadsafe</p>
<pre><code>import asyncio,time,threading
...
<ul>
<li>asyncio.run_coroutine_threadsafe(coro, loop):
<ul>
<li>此方法提交一个协程任务到循环中，loop 作为参数</li>
<li>返回 Future 供查询结果</li>
<li>当事件循环运行时, 必须在不同线程下添加协程任务到此循环中</li>
<li>其内部也用到了 call_soon_threadsafe</li>
</ul>
</li>
</ul>
<p>##由异步爬虫引发的协程问题</p>
...
<ul>
<li>long_calc：模拟高耗时的同步处理</li>
<li>waiting_task：模拟有时间限制的异步处理，时间设为 1 秒，时限设为 1.5 秒。</li>
</ul>
<p>运行三个异步，一个同步，结果是这样的：</p>
<pre><code>waitting task 0 start
