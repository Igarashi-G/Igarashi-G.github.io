<template><div><p>一、字段：
AutoField(Field) - int 自增列，必须填入参数 primary_key=True（注意 MySQL 中只能有一个自增列，因此 Django 中设定必须如此，此时便没有默认的主键自增列了）</p>
<pre><code>BigAutoField(AutoField)
...
unique_for_year 数据库中字段【年】部分是否可以建立唯一索引</p>
<pre><code>以下是对于Django admin中的有影响的字段
...
<p>四、拓展知识 1.触发 Model 中的验证和错误提示有两种方式：
a. Django Admin 中的错误信息会优先根据 Admiin 内部的 ModelForm 错误信息提示，如果都成功，才来检查 Model 的字段并显示指定错误信息
b. 调用 Model 对象的 clean_fields 方法，如： # <a href="http://models.py" target="_blank" rel="noopener noreferrer">models.py<ExternalLinkIcon/></a>
class UserInfo(models.Model):
nid = models.AutoField(primary_key=True)
username = models.CharField(max_length=32)</p>
<pre><code>            email = models.EmailField(error_messages={'invalid': '格式错了.'})
...
b. 与之关联的值设置为可执行对象的返回值，设置：models.SET(可执行对象)</p>
<pre><code>                                                    def func():
                                                        return 10
...
<p>2、进阶操作（了不起的双下划线）补充：
isnull # Entry.objects.filter(pub_date__isnull=True)</p>
<pre><code>range
    #
