...
<p><code v-pre>Docker</code> 在 <strong>1.8</strong> 版本之前，全部是封装的 <a href="https://linuxcontainers.org/lxc/introduction/" target="_blank" rel="noopener noreferrer">LXC<ExternalLinkIcon/></a>，一个 <strong>用户态</strong> 使用容器化特性的 <strong>接口</strong>（调用 Kernel），但不具备跨平台能力</p>
<p>随后为了实现跨平台，抽出了 <a href="https://linuxcontainers.org/lxc/introduction/" target="_blank" rel="noopener noreferrer">libcontainer<ExternalLinkIcon/></a> 项目，把 <code v-pre>namespace</code>、<code v-pre>cgroup</code> 的操作封装在该项目里，支持不同的平台类型</p>
...
<h5 id="虚拟机" tabindex="-1"><a class="header-anchor" href="#虚拟机" aria-hidden="true">#</a> 虚拟机：</h5>
<p>如 <strong>VMware</strong> 、<strong>PVE</strong>、<strong>ESXi</strong>、<strong>Workstation</strong> 等，多台虚拟机都虚拟出了一套 <strong>不同</strong> 的 <strong>虚拟机器硬件资源</strong>、<strong>Kernel</strong>（内核）、<strong>Lib 库</strong>，然后在上层运行各自的 APP，像是物理机的系统中的子系统一样，从物理虚拟层面进行隔离，占用资源极高</p>
<ul>
<li><code v-pre>Hypervisor:</code> 一种运行在基础物理服务器和操作系统之间的中间软件层，可允许多个操作系统和应用共享硬件。</li>
</ul>
<h5 id="容器" tabindex="-1"><a class="header-anchor" href="#容器" aria-hidden="true">#</a> 容器：</h5>
...
<ul>
<li><code v-pre>Container Runtime：</code> 通过 Linux 内核虚拟化能力管理多个容器，多个容器共享一套操作系统内核。因此 <strong>摘掉了内核占用的空间</strong> 及运行所需要的耗时，使得容器极其轻量与快速。</li>
</ul>
<blockquote>
<p>因此，<strong>Docker</strong>用于解决如下需求：</p>
<ul>
<li>环境、依赖不一致</li>
<li>物理硬件资源不够</li>
<li>快速交付介质，直接交付打包后的 <code v-pre>Docker</code> 镜像，各平台部署</li>
<li>跨平台，方便装任何系统，屏蔽平台间差异</li>
<li>物理资源相互隔离（也可以做到内存、CPU 等资源分配与隔离，但安全性不如虚拟机）</li>
<li><code v-pre>Docker</code> 启动多容器生命周期管理</li>
</ul>
</blockquote>
<h3 id="_1-4-docker-架构" tabindex="-1"><a class="header-anchor" href="#_1-4-docker-架构" aria-hidden="true">#</a> 1.4 Docker 架构</h3>
<p>2015 年 6 月，<code v-pre>Docker</code> 成立 <a href="https://opencontainers.org/" target="_blank" rel="noopener noreferrer">OCI<ExternalLinkIcon/></a> （Open Container Initiative 开放容器计划）组织，建立通用标准并由该组织维护 <code v-pre>libcontainer</code> 项目，后续由从仅包含 <code v-pre>kernel</code> 的库加入了 <strong>CLI</strong> 工具且改名为 <a href="https://github.com/opencontainers/runc" target="_blank" rel="noopener noreferrer">runC<ExternalLinkIcon/></a> （运行容器的轻量级工具）</p>
<p><code v-pre>Docker</code> 随后做出了架构调整：</p>
...
<ul>
<li><code v-pre>runC:</code> 是一个 <code v-pre>linux</code> <strong>CLI</strong>（<em>命令行</em>）工具， <code v-pre>runC + containerd-shim</code> 通过 <code v-pre>gRPC</code> 去调用 <code v-pre>containerd</code> 来创建和运行容器</li>
<li><code v-pre>containerd：</code> 一个守护程序，它管理容器的生命周期，屏蔽了 <code v-pre>docker deamon</code> 底层细节（同时解耦升级后的不兼容），抽象出了一套 <code v-pre>gRPC</code> 接口，提供了在节点上执行容器和管理镜像的最小功能集</li>
</ul>
<img src="@source/tool/Docker/img/CS架构.png">
<p>因此，<code v-pre>Docker</code> 演变为了 <strong>CS</strong> 架构的产品</p>
<ul>
<li><code v-pre>Client：</code> 由 <code v-pre>docker CLI</code> 通过 <code v-pre>REST API</code> 调用 <code v-pre>Server</code> 端服务</li>
<li><code v-pre>Server：</code> 为 <code v-pre>docker daemon</code> 守护进程</li>
</ul>
<hr>
...
<h6 id="必要-配置宿主机网卡转发" tabindex="-1"><a class="header-anchor" href="#必要-配置宿主机网卡转发" aria-hidden="true">#</a> 必要：配置宿主机网卡转发</h6>
...
<li><code v-pre>net.ipv4.ip_forward = 1</code>: 比较重要控制机器间 <strong>网卡流量的传递开关</strong> 需要打开，否则一个机器多块网卡无法传递流量</li>
</ul>
<h5 id="yum-安装配置-docker" tabindex="-1"><a class="header-anchor" href="#yum-安装配置-docker" aria-hidden="true">#</a> yum 安装配置 docker</h5>
<ul>
<li>
<p>安装 <code v-pre>yum-utils</code> 包（提供 <code v-pre>yum-config-manager</code> 实用程序）</p>
...
<li>
<p>查看安装的 <code v-pre>docker</code> 软件</p>
...
<li>
<p>安装 <code v-pre>Docker</code> 的阿里 <code v-pre>GPG</code> 密钥证书</p>
...
<li>
<p>写入 <code v-pre>Docker</code> 镜像源地址</p>
...
<li>
<p>再更新，并安装 <code v-pre>docker</code></p>
