<template><div><p>RESTful 规范介绍：
初识 RESTful 架构：
越来越多的人开始意识到，网站即软件，而且是一种新型的软件。
5G 的腾空出世，用发展的眼光来看的话。下载一个 2GB 的电影仅需要 1、2 秒，此时往本地方硬盘的意义就不大了。网上读取比本地硬盘还快，
我为什么要放硬盘啊？</p>
<pre><code>    这种&quot;互联网软件&quot;采用浏览器/服务器模式。分布式的体系，通过互联网通信，具有高延时（high latency）、高并发等特点。（现在肯定比本地慢很多）
...
<p>Restful API 设计指南：(如何设计一套合理、好用的 API)
一、协议
API 与用户通信，总是使用 HTTPs 协议（通信加密，就算被劫持了，也看不见数据，主流，运维负责）</p>
<pre><code>二、域名
    应该尽量将API部署在专用域名之下。
