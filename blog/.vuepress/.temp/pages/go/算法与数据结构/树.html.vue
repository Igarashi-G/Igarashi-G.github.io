...
<p>在任意一棵非空的树中：</p>
<ul>
<li>有且仅有一个特定的 <strong>根结点</strong>（<em>Root</em>）</li>
<li>当 <code v-pre>n&gt;1</code> 时，其余结点可分为 <code v-pre>m（m&gt;0）</code>个树，可以称为根的 <strong>子树</strong>（<em>SubTree</em>）</li>
</ul>
<p>如图所示</p>
...
<img src="@source/go/算法与数据结构/img/tree-03.png">
<p>树的相关术语：</p>
<ul>
<li><strong>父结点</strong>：下方连接多个结点</li>
<li><strong>子结点</strong>：父结点下的结点</li>
<li><strong>结点的度</strong>（<em>De-gree</em>）：拥有的子树数</li>
<li><strong>叶结点</strong>（<em>Leaf</em>）：没有子结点的结点，即 <strong>度为 0</strong>，也称为 <strong>终端结点</strong></li>
<li><strong>非叶结点</strong>：<strong>度不为 0</strong> 的结点，也称为 <strong>分支结点</strong>，也可以称为内部结点（<em>根结点除外</em>）</li>
<li><strong>树的深度</strong>：从上往下定义，为叶结点所在 <strong>最大层数</strong></li>
</ul>
<img src="@source/go/算法与数据结构/img/tree-04.png">
<p><strong>结点的层次</strong>（<em>Level</em>）：根结点作为第一层，其孩子为第二层</p>
<img src="@source/go/算法与数据结构/img/tree-05.png">
<p>有序树与无序树：</p>
<ul>
...
<ul>
<li><strong>缺点解决</strong>：增加一个结点的 <strong>最左孩子域</strong>，这样就能轻易得到结点的孩子
<ul>
<li>如果没有子结点，整个最左孩子域设置为 <strong>-1</strong></li>
<li>但是如果孩子很多，超过 <strong>2</strong> 个，又需要关注结点的双亲、孩子、兄弟，那么需要不断的扩展结构，添加其他的域，比如：<strong>双亲域、左右兄弟域</strong> 等等</li>
</ul>
</li>
</ul>
...
<blockquote>
<p>相当于用树的结构体中的 <strong>nodes</strong>，把每个结点像数组一样存起来，再额外加指针，关联（<em>链式指向</em>）其所有子结点，每个结点通过数组下标再次关联，形成完整结构（<em>感觉本质还是以顺序存储为媒介，混合了链式关联的结构</em>）</p>
</blockquote>
<p>可以将两种表示方法合二为一，存储效果如图：</p>
<img src="@source/go/算法与数据结构/img/tree-06-3.png">
<blockquote>
...
<p><strong>注意</strong>：二叉树的 <strong>左子树和右子树是有顺序的，不能任意颠倒</strong>，即使树种某个结点只有一棵子树，也要区分它是左子树还是右子树。交换左右子树后生成的新二叉树就不再是以前的二叉树了！！<strong>所以二叉树是 有序树！</strong></p>
<p>一些特殊的二叉树：</p>
<ul>
<li><strong>斜树</strong>：斜树看起来类似线性结构，有左斜树（<em>只有左侧结点</em>），右斜树（<em>只有右侧结点</em>），此时 <strong>结点的个数就是二叉树的深度</strong></li>
...
<p><strong>注意</strong>：<strong>完全二叉树不一定是满的</strong>，但是只要将完全二叉树补充上对应子结点，就是一棵满二叉树。</p>
<p>也可以推理出：</p>
<ul>
<li>满二叉树一定是完全二叉树</li>
...
<li>度为 <strong>1</strong> 的结点要么是 <strong>1</strong> 个，要么是 <strong>0</strong> 个</li>
<li>同样结点数的二叉树，<strong>完全二叉树深度最小</strong></li>
<li>如果结点度为 <strong>1</strong>，则该结点只有孩子结点，不存在只有右子树的情况</li>
</ul>
<p>假设完全二叉树的高度为 <strong>h</strong> ，<strong>h &gt;= 1</strong>，则：</p>
<ul>
<li>则至少有 <strong>2<sup>h-1</sup></strong> 个结点</li>
<li>则最多有 <strong>2<sup>h</sup> - 1</strong> 个结点 (<em>满二叉树</em>)</li>
</ul>
<p>总结点数量 n ：</p>
<ul>
<li>则：<strong>2<sup>h - 1</sup> &lt;= b &lt; 2<sup>h</sup></strong></li>
...
<li><strong>2i&lt;=n</strong>，其左子结点编号为 <strong>2i</strong></li>
<li><strong>2i&gt;n</strong>，则其无左子结点</li>
...
<p>通过前序/后续遍历 + 中序遍历的结果也可以推导出唯一的一棵二叉树：</p>
<ul>
...
<li>中缀表达式（<em>infix expression</em>），该方式符合人类的视觉思维</li>
<li>后缀表达式（<em>postfix express</em>），又称为逆波兰表达式</li>
</ul>
...
<img src="@source/go/算法与数据结构/img/bstree-02.svg">
<p>二叉搜索树与哈希表作为查找时的对比：</p>
<ul>
<li>哈希表需要一个很大的数组，会造成一定的空间浪费</li>
...
<ul>
<li><code v-pre>node.left == nil &amp;&amp; node.parent == nil</code>
<ul>
<li>前驱为：<strong>无前驱结点</strong></li>
</ul>
</li>
<li><code v-pre>node.left == nil &amp;&amp; node.parent != nil</code>
<ul>
<li>前驱为：<strong>node.parent.parent.parent....</strong></li>
<li>终止条件为：<strong>node</strong> 在 <strong>parent</strong> 的右子树中</li>
</ul>
</li>
<li><code v-pre>n.left != nil</code>
<ul>
<li>前驱为：<strong>node.left.right.right.right...</strong></li>
<li>终止条件为：<strong>right</strong> 为 <strong>nil</strong></li>
</ul>
</li>
</ul>
...
<ul>
<li><code v-pre>node.right == nil &amp;&amp; node.parent == nil</code>
<ul>
<li>后继为：<strong>无后继结点</strong></li>
</ul>
</li>
<li><code v-pre>node.right == nil &amp;&amp; node.parent != nil</code>
<ul>
<li>后继为：<strong>node.parent.parent.parent....</strong></li>
<li>终止条件为：<strong>node</strong> 在 <strong>parent</strong> 的左子树中</li>
</ul>
</li>
<li><code v-pre>n.right != nil</code>
<ul>
<li>后继为：<strong>node.right.right.right.right...</strong></li>
<li>终止条件为：<strong>left</strong> 为 <strong>nil</strong></li>
</ul>
</li>
</ul>
<div class="language-go ext-go line-numbers-mode"><pre v-pre class="language-go"><code>
...
<li>度为 <strong>1</strong> 结点： 使用子结点替换原结点，如果删除的结点也是根结点，则还需将根结点指针指向子结点
<ul>
<li>如果删除的结点只有左子结点：<strong><code v-pre>child.parent = node.parent node.parent.left = child</code></strong></li>
<li>如果删除的结点只有右子结点：<strong><code v-pre>child.parent = node.parent node.parent.right = child</code></strong></li>
</ul>
</li>
<li>度为 <strong>2</strong> 结点： 使用期前驱/后继替换当前结点，然后删除刚才替换的前驱/后继</li>
</ul>
...
<li>删除结点 <strong>7</strong>：有两种方式
<ul>
<li>左侧查找，用 <strong>5</strong> 替换位置 <strong>7</strong>，此时 <strong>3</strong> 依然指向 <strong>5</strong>，<strong>5</strong> 的 <strong>right</strong> 需要指向 <strong>9</strong></li>
<li>右侧查找，用 <strong>8</strong> 替换位置 <strong>7</strong>，此时 <strong>8</strong> 的 <strong>left</strong> 是 <strong>5</strong>，<strong>right</strong> 是 <strong>9</strong></li>
</ul>
</li>
<li>删除结点 <strong>15</strong>：也有从左侧、右侧查找两种方式
<ul>
<li>右侧查找：用 <strong>18</strong> 替换位置 <strong>15</strong>，<strong>20</strong> 的 <strong>left</strong> 指向 <strong>19</strong></li>
</ul>
</li>
</ul>
<div class="language-go ext-go line-numbers-mode"><pre v-pre class="language-go"><code>
...
<p>常见的平衡二叉搜索树有：</p>
<ul>
<li><strong>AVL 树：</strong> <strong>Windows</strong> 操作系统常用</li>
