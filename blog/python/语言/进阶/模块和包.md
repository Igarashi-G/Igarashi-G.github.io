...
[标准库:](/back_end/python/package/标准库基础/时间) **Python** 自带的标准模块，内嵌到编译器中，提供操作系统等的基本调用接口，如 `sys`

对于非内嵌库（_自定义_），当模块导入后（如: `import fibo`），解释器首先搜索具有 `fibo` 名称的内置模块，`sys.builtin_module_names` 中，若没找到，则会在 `sys.path` 给出的目录列表中搜索
...
- `version` 一般为 **Python** 的版本号
  ...
  - 如，**CPython** 的 **3.3** 发行版，`spam.py` 的编译版本缓存为 ` __pycache__``module.*version*.pyc``__pycache__/spam.cpython-33.pyc `
...
包支持一个更特殊的属性 [`__path__`](https://docs.python.org/zh-cn/3.10/reference/import.html#path__) ，在 `__init__.py` 文件的代码被执行前，该属性会被初始化为自身所在的目录的列表
...
- `-c` 配置文件的路径，会是用了 [argparse]()（_参数解析器_）来获取执行脚本需要的参数
- `-r` 默认 `True` ，用于初始化的判断
